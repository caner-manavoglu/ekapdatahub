...
const clickMahkemeKararlariTab = async (driver) => {
  const tab = await driver.wait(async () => {
    const tabs = await getVisibleElements(driver, 'div#tab-item.tab-item, div.tab-item');
    for (const item of tabs) {
      const text = ((await item.getText()) || '').trim();
      const normalized = normalizeMonthText(text);
      if (normalized.includes('mahkeme') && normalized.includes('karar')) {
        return item;
      }
    }
    return null;
...
const run = async () => {
  ...
  try {
    ...
    await clickMahkemeKararlariTab(driver);
